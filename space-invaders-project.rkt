...
(define I4 (make-invader 150 15 -1.5))
...
(define G4 (make-game LOI4 LOM2 T0))
...
#;
(define (movePieces s)
  (make-game ((sepInvaderX (game-invaders s))
              (moveMissile (game-missiles s))
...
(define (movePieces s)
  (make-game
;   (sepInvaderY (sepInvaderX (hitDetection (game-invaders s) (game-missiles s))))
   (sepInvaderY (sepInvaderX (game-invaders s)))
...
;(define (hitDetection LOII LOMM) empty)
...
(define (hitDetection LOII LOMM)
  (cond [(empty? LOII) LOII]
        [(empty? LOMM) LOII]
        [(cond [(equal? (invader-x (first LOII)) (missile-x (first LOMM)))
                (cond [(equal? (invader-y (car LOII)) (missile-y (first LOMM)))
                       (remove (first LOII) LOII)]
                      [(equal? (invader-y (first LOII)) (missile-y (sepMissileHitDetection (rest LOMM))))
                       (remove (first LOII) LOII)]
                      [else LOII])]
               [(empty? (rest LOII)) LOII]
               [else (hitDetection (rest LOII) LOMM)])
         LOII]
        [else LOII]))
                   


(define (sepMissileHitDetection LOMM)
  (cond [(empty? LOMM) (make-missile 0 0)]
        [else (car LOMM)]))
...
#;
[(= (invader-x (first LOII)) (missile-x (first LOMM)))
 (remove (first LOII) LOII)]
;[else LOII]))
                      

;; LiseofInvader -> ListofInvader
;; Interp. Consumes a ListofInvader and moves each invader to the next location based on its current position and speed
;(check-expect (sepInvaderX LOI4) (cons (make-invader 201.5 31.5 1.5) (cons (make-invader 148.5 16.5 -1.5) empty)))

;(define (sepInvaderX LOI) 0)

#;
(define (sepInvaderX LOI)
  (cond [(empty? LOI) empty]
        [else (moveInvaderX (first LOI))
              (sepInvaderX (rest LOI))]))


(define (sepInvaderX LOI)
  (cond [(empty? LOI) empty]
        [else
         (cons (moveInvaderX (first LOI))
               (sepInvaderX (rest LOI)))]))

;; Invader -> Invader
;; Consumes a single invader and changes its location on the screen by the invader speed constants

;(define (moveInvaderX i) 0)

;(check-expect (moveInvaderX (first LOI4)) (make-invader 201.5 31.5 1.5))

(define (moveInvaderX i)
  (cond [(> 0 (invader-dx i))
         (moveInvaderNegSpeedX i)]
        [else
         (moveInvaderPosSpeedX i)]))


(define (moveInvaderNegSpeedX i)
  (cond [(= 0 (invader-x i)) (make-invader (+ (invader-x i) INVADER-X-SPEED) (invader-y i) 1.5)]
        [else (make-invader (- (invader-x i) INVADER-X-SPEED) (invader-y i) (invader-dx i))]))
  


(define (moveInvaderPosSpeedX i)
  (cond [(= WIDTH (invader-x i))(make-invader (- (invader-x i) INVADER-X-SPEED) (invader-y i) -1)]
        [else (make-invader (+ (invader-x i) INVADER-X-SPEED) (invader-y i) (invader-dx i))]))
...
;(check-expect (sepInvaderY LOI4) (cons (make-invader 201.5 31.5 1.5) (cons (make-invader 148.5 16.5 -1.5) empty)))

;(define (sepInvaderY LOI) 0)


(define (sepInvaderY LOI)
  (cond [(empty? LOI) empty]
        [else
         (cons (moveInvaderY (first LOI))
               (sepInvaderY (rest LOI)))]))

;; Invader -> Invader
;; Consumes a single invader and changes its location on the screen by the invader speed constants
;(define (moveInvaderY i) 0)

(define (moveInvaderY i)
  (cond [(> 0 (invader-dx i))
         (moveInvaderNegSpeedY i)]
        [else
         (moveInvaderPosSpeedY i)]))


(define (moveInvaderNegSpeedY i)
  (make-invader (invader-x i) (- (invader-y i) (invader-dx i)) (invader-dx i)))

(define (moveInvaderPosSpeedY i)
  (make-invader (invader-x i) (+ (invader-y i) (invader-dx i)) (invader-dx i)))
